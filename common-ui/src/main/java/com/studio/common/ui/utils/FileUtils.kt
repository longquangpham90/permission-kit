...
object FileUtils {
    ...
    fun Context.getSelectedFileFromResult(data: Intent?): File? {
        return if (data != null) {
            val fileUri = data.data ?: return null
            val filePath = getPath(this, fileUri) ?: return null
    ...
    @JvmStatic
    fun getAppCacheDirectory(context: Context): File = context.externalCacheDir ?: context.cacheDir

    @JvmStatic
    fun getAppCacheDirectoryPath(context: Context): String = getAppCacheDirectory(context).absolutePath

    @SuppressLint("UsableSpace")
    @JvmStatic
    fun isLowStorage(context: Context): Boolean = getAppCacheDirectory(context).usableSpace < MINIMUM_FREE_SPACE
    ...
    fun saveImageToExternalStoragePic(
        context: Context,
        finalBitmap: Bitmap?
    ): File? {
        val file: File
    ...
    fun openFile(
        ...
        try {
            ...
            if (url.toString().contains(".doc") || url.toString().contains(".docx")) {
                ...
                // JPG file
                intent.setDataAndType(uri, MIME_TYPE_JPEG)
            } else if (url.toString().contains(".txt")) {
                // Text file
                intent.setDataAndType(uri, MIME_TYPE_TEXT)
                ...
                // Video files
                intent.setDataAndType(uri, MIME_TYPE_VIDEO)
            } else {
    ...
    fun getVideoIntro(
        ...
        val dirPath =
            if (Environment.MEDIA_MOUNTED == Environment.getExternalStorageState()) {
                context.externalCacheDir.toString() + File.separator + dirName
            } else {
                context.cacheDir.toString() + File.separator + dirName
            }
        ...
        val outputName = "$fileNamePrefix$timeStamp.mp4"
